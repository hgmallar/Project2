...
function assignPlayer() {
  ...
  socket.on("player assignments", function (data) {
    ...
    if (playerNumber > 2) {
      ...
      $(".modal-body").text(
        "There are already two players logged in and playing the game. You are watching the game and will be able to play when a user logs out."
      );
      $("#myModal").modal("show");
      playerState = "hold";
    }
...
function checkWins() {
  if (
    ...
    ($("#00").text() === opponentMark &&
      $("#01").text() === opponentMark &&
      $("#02").text() === opponentMark) ||
    ($("#10").text() === opponentMark &&
      $("#11").text() === opponentMark &&
      $("#12").text() === opponentMark) ||
    ($("#20").text() === opponentMark &&
      $("#21").text() === opponentMark &&
      $("#22").text() === opponentMark) ||
    ($("#00").text() === opponentMark &&
      $("#11").text() === opponentMark &&
      $("#22").text() === opponentMark) ||
    ($("#02").text() === opponentMark &&
...
socket.on("game in play", function (data, playerNames) {
  ...
  $("#wins1").text(wins1);
  $("#wins2").text(wins2);
  profpoke1 = playerNames[0].userpoke;
  profpoke2 = playerNames[1].userpoke;
